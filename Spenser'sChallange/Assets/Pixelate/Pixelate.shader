	...
	SubShader{
		...
		Pass{
			...
			fixed4 frag(v2f i):SV_Target{
				//float4 rescol = tex2D(_MainTex, i.uv);
				float2 coord;
				float2 pixelSize = 1 / float2(_ScreenParams.x, _ScreenParams.y);
				pixelSize.x *= _PixelateX;
				pixelSize.y *= _PixelateY;
				coord.x =floor( i.uv.x/ pixelSize.x) * pixelSize.x;
				coord.y = floor(i.uv.y /pixelSize.y) * pixelSize.y;
				float4 rescol = tex2D(_MainTex, coord);
				return rescol;
			}
